...
def run_counts(p, api_entry=True):
    ...
    sp.run(  # Use subprocess run rather than Popen as complex call is a PITA
        f"python3 -m app.src.parse_bam -Mode parse -SeqName {p['SeqName']} -ExpDir {p['ExpDir']} -ExpName {p['ExpName']} -SingleEnded {single_ended} | sort | uniq -c | sed s'/ /,/'g | sed s'/^[,]*//'g > experiments/{p['ExpName']}/{p['SeqName']}_PosCounts.csv", shell=True)
