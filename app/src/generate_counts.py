...
def run_counts(p):
    ...
    in_file = f"{p['ExpRoot']}/{p['ExpName']}/{p['ExpName']}.bam"
    bamview_fname = f"{p['ExpRoot']}/{p['ExpName']}/{p['ExpName']}_bamview.txt"
    ...
    sp.run(  # Use subprocess run rather than Popen as complex call is a PITA
        f"python3 -m app.src.parse_bam -Mode parse -SeqName {p['ExpName']} -ExpDir {p['ExpDir']}/ -ExpName {p['ExpName']} -SingleEnded {p['SingleEndedReads']} -ExpRoot {p['ExpRoot']} -MatchLength {p['MatchLength']} | sort | uniq -c | sed s'/ /,/'g | sed s'/^[,]*//'g > {p['ExpRoot']}/{p['ExpName']}/{p['ExpName']}_PosCounts.csv", shell=True)
