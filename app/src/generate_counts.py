...
def run_counts(p):
    ...
    in_file = f"experiments/{p['ExpName']}/{p['ExpName']}.bam"
    ...
    sp.run(  # Use subprocess run rather than Popen as complex call is a PITA
        f"python3 -m app.src.parse_bam -Mode parse -SeqName {p['ExpName']} -ExpDir {p['ExpDir']}/ -ExpName {p['ExpName']} -SingleEnded {p['SingleEndedReads']} | sort | uniq -c | sed s'/ /,/'g | sed s'/^[,]*//'g > experiments/{p['ExpName']}/{p['ExpName']}_PosCounts.csv", shell=True)
